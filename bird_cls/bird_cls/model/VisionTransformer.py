...
class GlobalLayerNorm(nn.Module):
    ...
    def forward(self, y):
        ...
        mean = y.mean(dim=[2, 3], keepdim=True)  # [M, 1, 1]
        var = (torch.pow(y - mean, 2)).mean(dim=[2, 3], keepdim=True)
...
class ShuffleTransformer(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, token_dim=32, embed_dim=96, mlp_ratio=4.,
        ...
        super().__init__()
        self.norm = GlobalLayerNorm(channel_size=3)
...
if __name__ == '__main__':
    ShuffleTransformer(img_size=224,in_chans=3,num_classes=3)